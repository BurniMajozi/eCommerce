-- Return per-scope MFA-required capabilities so the backend can enforce step-up
-- automatically from capability metadata, instead of each route author manually
-- passing an mfa flag (which risks drift for a privileged capability).
...
comment on function public.resolve_access_scope(uuid, uuid, uuid) is
  'Server-only immutable tenant scope. Returns tenant + global platform roles, their capabilities, and the subset of capabilities that require MFA (aal2).';
