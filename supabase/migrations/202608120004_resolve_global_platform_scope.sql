-- Include tenant-independent platform roles in Medusa's server-only scope
-- result without collecting membership roles from unrelated tenants.
...
comment on function public.resolve_access_scope(uuid, uuid, uuid) is
  'Server-only immutable tenant scope including tenant-specific and global platform roles.';
